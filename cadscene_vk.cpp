...
void CadSceneVK::init(const CadScene& cadscene, VkDevice device, VkPhysicalDevice physicalDevice, VkQueue queue, uint32_t queueFamilyIndex)
{
  ...
  m_buffers.indirects = m_memAllocator.createBuffer((m_meshletTotal+cadscene.m_objects.size()) * sizeof(VkDrawIndirectCommand),
...
#if SW_MESHLET
  m_infos.indirects       = {m_buffers.indirects, 0, (m_meshletTotal+ cadscene.m_objects.size()) * sizeof(VkDrawIndirectCommand)};
#endif
